...
import React, { useState, useEffect, useRef } from 'react';
import { nodeTypeNames, nodeConnections } from './nodeRules';
import blankFlowNodeOptions from './blankFlowNodeOptions';
...
const NodeOptions = ({ id, onOptions, nodeType, allowedConnections = [], flowType }) => {
  ...
  // Get the allowed connections for this node type
  const validConnections = flowType === 'blank'
    ? (blankFlowNodeOptions[nodeType] || [])
    : (allowedConnections.length > 0 ? allowedConnections : []);

  // Get all available node types from nodeTypeNames
  const allNodeTypes = Object.keys(nodeTypeNames);
  
  // Sort node types with allowed connections first
  const sortedNodeTypes = [...allNodeTypes].sort((a, b) => {
    const aIsAllowed = validConnections.includes(a);
    const bIsAllowed = validConnections.includes(b);
        ...
        >
          ...
          {showSubMenu && (
            ...
            >
              ...
              {/* Show enabled/allowed node types first */}
              {validConnections.length > 0 && (
                <>
                  ...
                  {sortedNodeTypes
                    .filter(nodeTypeOption => validConnections.includes(nodeTypeOption))
                    .map(nodeTypeOption => {
                      const friendlyName = nodeTypeNames[nodeTypeOption] || nodeTypeOption;
                  ...
                  {/* Separator between enabled and disabled nodes */}
                  {sortedNodeTypes.some(type => !validConnections.includes(type)) && (
                    <div style={{ height: '1px', background: '#ddd', margin: '8px 0' }}></div>
                  )}
                </>
              )}
              
              {/* Show disabled node types after the enabled ones */}
              {sortedNodeTypes.some(type => !validConnections.includes(type)) && (
                <div style={{ fontSize: '13px', color: '#666', marginBottom: '5px', fontWeight: 'bold' }}>
                  Not Available:
                </div>
