...
const Sidebar = ({ selectedNode, isOpen, toggleSidebar, setNodes, setEdges, edges, nodes, flowType }) => {
  ...
  const departmentsList = ['IT', 'FINANCE', 'HR', 'OTHERS'];
  const acceptedFormats = '.pdf,.docx';
  const apiUrl = 'http://127.0.0.1:5100/api/upload';
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedFiles.length === 0 || !selectedDepartment) {
      setStatus({
        message: 'Please select both a department and at least one file',
        isError: true,
      });
      return;
    }
              ...
              <>
                ...
                {selectedNode.type === 'rag' && (
                  <div className="section" style={{ marginBottom: '20px' }}>
                    ...
                    >
                      {expandedSections.knowledgeRepository ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                      <h3 style={{ margin: '0 0 0 5px' }}>Knowledge Repository</h3>
                    </div>

                    {expandedSections.knowledgeRepository && (
                      <div className="node-control-group">
                        <div
                          style={{
                            padding: '1rem',
                        ...
                        >
                          ...
                          >
                            ...
                            {selectedFiles.length > 0 && (
                              ...
                              >
                                Selected files:
                                <ul style={{ paddingLeft: '20px', margin: '0.5rem 0 0 0' }}>
                            ...
                            <button
                              type="submit"
                              disabled={isUploading || selectedFiles.length === 0 || !selectedDepartment}
                              style={{
                                padding: '0.8rem',
                                backgroundColor:
                                  isUploading || selectedFiles.length === 0 || !selectedDepartment ? '#ccc' : '#007bff',
                                color: 'white',
                                border: 'none',
                                borderRadius: '4px',
                                fontSize: '1rem',
                                cursor:
                                  isUploading || selectedFiles.length === 0 || !selectedDepartment
                                    ? 'not-allowed'
                                    : 'pointer',
                              }}
                            >
